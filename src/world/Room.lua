...
function Room:generateObjects()
    ...
    local numpots = math.random(20)
    for x = numpots, 1, -1 do
        table.insert(self.objects, GameObject(
        GAME_OBJECT_DEFS['pot'],
        math.random(MAP_RENDER_OFFSET_X + TILE_SIZE,
                    VIRTUAL_WIDTH - TILE_SIZE * 2 - 16),
...
function Room:update(dt)
    ...
    for k, object in pairs(self.objects) do
        if object.type == 'pot' and object.state == 'thrown' then
            ...
            if self:wallsCollided(object) then
                table.remove(self.objects, k)
            end
...
function Room:wallsCollided(object) 
    if object.x <= TILE_SIZE or object.x >= TILE_SIZE * self.width
       or object.y <= TILE_SIZE or object.y >= TILE_SIZE * self.height then
        return true
    end
